...
SELECT 
	category, 
	gender, 
	COUNT(*) AS No_Of_Trans FROM retail_sales
GROUP BY 
	gender, 
	category
ORDER BY No_Of_Trans DESC;

Q.7 --- Find the average age of customers.
SELECT 
	ROUND(AVG(age),1) 
FROM retail_sales; 

Q.8 --- Find categories with more than 500 transactions.
SELECT 
	category, 
...
Q.9 --- Find genders whose total sales exceed ₹100,000.
SELECT 
	gender, 
	SUM(total_sale) AS Exceed_lakh 
FROM retail_sales
GROUP BY gender
HAVING SUM(total_sale) > 100000
LIMIT 1;

Q.10 --- Find categories where the average sale amount is greater than ₹500.
SELECT 
	category,
...
Q.11 --- Find customers who made more than 5 purchases and count how many data is there:
...
Q.12 --- Find months where total revenue exceeded ₹50,000.
SELECT 
...
Q.16 --- Write a SQL query to create each shift and number of orders (Example Morning <12, Afternoon Between 12 & 17, Evening >17):
WITH hourly_sale
AS(
SELECT *,
	CASE 
